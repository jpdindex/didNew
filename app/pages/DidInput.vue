...
const rangeEndId = ref<string | null>(null)
const rangeEditingOpen = ref(false)
const rangeDelta = ref(0)
function toggleRangeMode() { rangeMode.value = !rangeMode.value; rangeStartId.value = null; rangeEndId.value = null; rangeEditingOpen.value = false; rangeDelta.value = 0 }
const rangeIndices = computed(() => {
  if (!rangeStartId.value || !rangeEndId.value) return null
  const ids = visibleRecords.value.map(r => r.id)
  const a = ids.indexOf(rangeStartId.value), b = ids.indexOf(rangeEndId.value)
  if (a < 0 || b < 0) return null
  return [Math.min(a, b), Math.max(a, b)] as const
})
const rangeSelectedIds = computed(() => {
  const idx = rangeIndices.value
  if (!idx) return new Set<string>()
  return new Set(visibleRecords.value.slice(idx[0], idx[1] + 1).map(r => r.id))
})
function tapRangeRow(id: string) {
  if (rangeEditingOpen.value) return
...
function startPress(id: string, isTouch = false) {
  ...
  if (rangeMode.value) { if (rangeIndices.value) pressTimer = setTimeout(() => { longPressed = true; openRangeEdit() }, 500); return }
    ...
    <div class="frameViewport">
      <div class="frame" :class="{ mirrored: game.mirrored }">
        <section class="left" :style="{ paddingBottom: (tableExpanded ? 380 : 250) + 'px' }">
          ...
          <div v-if="isEditMode" class="tableBar">
